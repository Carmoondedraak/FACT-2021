...
def train_srbm():
    ...
    epochs = (len(layer_dimensions) - 1) * epochs_per_layer
    ...
    for epoch in range(epochs):
        ...
        print(f'Epoch: {epoch}/{epochs}, loss: {np.mean(loss_)}')

        if epoch%epochs_per_layer == 0 and epoch != 0:
            srbm.add_layer()
            layer_idx += 1
            print(f'added layer: {layer_idx}')
        

    torch.save(srbm.state_dict(), f'srbm_d{train_digit}.ckpt')
    print('Model saved as srbm.ckpt')
    return srbm

# Loads and returns a stacked RBM model
def load_srbm_model(name=f'srbm_d{train_digit}.ckpt'):
    srbm = StackRBMs(n_hs=layer_dimensions, layer_idx=3).to(device)
...
def eval_srbm_model(srbm):
    ...
    for i in range(6, len(layer_dimensions) -1):
        srbm.layer_idx = i
        srbm.curr_layer = i
        M, colormap = srbm.attmaps(v_outlier, temp=TEMP)
        save_image(make_grid(colormap.cpu().detach()), f'outlier_attmaps_d{test_digit}_layeridx{i}.png')
